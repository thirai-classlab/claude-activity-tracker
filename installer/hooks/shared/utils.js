...
function parseTranscript(transcriptPath) {
  ...
  // once per unique id. See docs/specs/001-transcript-dedup.md.
  ...
  for (const line of lines) {
    ...
    if (type === 'assistant') {
      ...
      const msgId = msg.id;
      const isDuplicateMessage = msgId && seenMessageIds.has(msgId);
      ...
      // Token usage — count exactly once per (message.id). Fallback to legacy
      // behavior (always add) when message.id is missing so older transcripts
      // without ids still work.
      const usage = msg.usage;
      if (usage && !isDuplicateMessage) {
      ...
      // message.id rows carry the same model/stop_reason anyway).
      if (msg.model) curTurnModel = msg.model;
      if (msg.stop_reason) curTurnStopReason = msg.stop_reason;

      // Mark the message.id as seen after we decided how to account for usage.
